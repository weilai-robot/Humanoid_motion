...
if [ -e "${NECK_PORT}" ]; then
    ...
    if [ ${neck_rc} -ne 0 ]; then
        if [ "${NECK_SERVO_REQUIRED:-0}" = "1" ]; then
            echo "[run.sh] 颈部舵机使能失败 (rc=${neck_rc}), NECK_SERVO_REQUIRED=1, 中止启动"
            exit 1
        fi
        echo "[run.sh][WARN] 颈部舵机使能失败 (rc=${neck_rc}), 继续启动 —— 头部可能松动!"
    fi
else
    echo "[run.sh][WARN] 未发现颈部舵机串口 ${NECK_PORT}, 跳过使能"
fi
